"""
Mask R-CNN
Train on the Corn dataset for Kernels and bare cob.
...
Usage: import the module (see Jupyter notebooks for examples), or run from
   ...
   """ #python corn.py train --dataset=C:\Users\shrin\Documents\Python_Scripts\Mask_RCNN\datasets\corn --weights=coco"""
	
    # Resume training a model that you had trained earlier
    #python corn.py train --dataset=C:\Users\shrin\Documents\Python_Scripts\Mask_RCNN\datasets\corn --weights=last
...
class CornConfig(Config):
    ...
    STEPS_PER_EPOCH = MAX_NUMBER_TRAIN_IMG / IMAGES_PER_GPU
    VALIDATION_STEPS = 21 / IMAGES_PER_GPU
...
class CornDataset(utils.Dataset):
    ...
    def load_corn(self, dataset_dir, subset):
        ...
        self.add_class("corn", 1, "Yellow Corn")
        self.add_class("corn", 2, "Bare corn cob")
       
        # Train or validation dataset?
        assert subset in ["train", "val", "test", "new_test"]
        ...
        #   'size': 100202
        # }
        annotations = json.load(open(os.path.join(dataset_dir, "via_region_data_3class.json")))
...
#                    if (r['region_attributes'])['Name'] != "Corn":
#                        print("Added!!")
#                        polygons.append({'polygon' : r['shape_attributes'], 'class' : r['region_attributes']})
                if subset == "train":
